...
def load_data():
    try:
        df = pd.read_csv('./cleaned_matiks_data.csv')
        ...
        st.error("Error: 'cleaned_matiks_data.csv' not found. Please ensure the file is included in the app directory.")
        st.stop()
...
def calculate_kpis(df):
    ...
    retention_day7['Signup_Date'] = retention_day7['Signup_Date'].astype(str)
    ...
    stickiness['Month'] = stickiness['Month'].astype(str)
...
# Plotly theme based on Streamlit theme
...
if page == "Approach":
    ...
    <dark><p><b>Visualizations:</b> Interactive Plotly charts display trends and breakdowns, with a professional layout for easy exploration.</p>
    ...
    with tab1:
        ...
        fig_dau = px.line(kpis['dau'], x='Date', y='DAU', title='Daily Active Users (DAU)', template=plotly_template)
        fig_dau.update_layout(
            paper_bgcolor='rgba(0,0,0,0)',
            plot_bgcolor='rgba(0,0,0,0)',
            font_color='#333333' if theme == 'light' else '#e0e0e0',
            title_font_color='#003087' if theme == 'light' else '#b3d4fc'
        )
        st.plotly_chart(fig_dau, use_container_width=True)
        
        fig_wau = px.line(kpis['wau'], x='Week', y='WAU', title='Weekly Active Users (WAU)', template=plotly_template)
        ...
        fig_mau = px.line(kpis['mau'], x='Month', y='MAU', title='Monthly Active Users (MAU)', template=plotly_template)
        ...
        fig_stickiness = px.line(kpis['stickiness'], x='Month', y='Stickiness_Ratio', title='Stickiness Ratio (DAU/MAU)', template=plotly_template)
    ...
    with tab2:
        ...
        fig_revenue = px.line(kpis['revenue_trend'], x='Signup_Month', y='Total_Revenue_USD', title='Revenue Trend by Signup Month', template=plotly_template)
        ...
        fig_device = px.bar(kpis['device_breakdown'], x='Device_Type', y='Total_Revenue', title='Revenue by Device Type', template=plotly_template)
        ...
        fig_segment = px.bar(kpis['segment_breakdown'], x='Subscription_Tier', y='Total_Revenue', title='Revenue by Subscription Tier', template=plotly_template)
        ...
        fig_game_mode = px.bar(kpis['game_mode_breakdown'], x='Game_Mode', y='Total_Revenue', title='Revenue by Game Mode', template=plotly_template)
        fig_game_mode.update_layout(
            paper_bgcolor='rgba(0,0,0,0)',
            plot_bgcolor='rgba(0,0,0,0)',
            font_color='#333333' if theme == 'light' else '#e0e0e0',
            title_font_color='#003087' if theme == 'light' else '#b3d4fc'
        ...
        fig_rev_session = px.bar(kpis['revenue_per_session'], x='Game_Title', y='Revenue_Per_Session', title='Revenue per Session by Game Title', template=plotly_template)
        fig_rev_session.update_layout(
            paper_bgcolor='rgba(0,0,0,0)',
            plot_bgcolor='rgba(0,0,0,0)',
            font_color='#333333' if theme == 'light' else '#e0e0e0',
            title_font_color='#003087' if theme == 'light' else '#b3d4fc'
    ...
    with tab3:
        ...
        fig_retention = px.line(kpis['retention_day7'], x='Signup_Date', y='Day7_Retention_Rate', title='Day 7 Retention Rate by Signup Month', template=plotly_template)
    ...
    with tab4:
        ...
        fig_cluster = px.scatter(kpis['df'], x='Total_Hours_Played', y='Total_Revenue_USD', color='Cluster', title='User Segmentation by Engagement and Revenue', template=plotly_template)
